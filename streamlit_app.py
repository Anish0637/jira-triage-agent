...
def _cognito_login(email: str, password: str) -> dict | None:
    """Returns auth tokens dict or None on failure."""
...
def _show_login():
    ...
    if submitted:
        if not _POOL_ID or not _CLIENT_ID:
            # Cognito not configured — allow bypass (dev mode)
...
def _show_new_password_form():
    ...
    if submitted:
        ...
        if result is None:
            st.error("Failed to set password. Make sure it meets requirements (8+ chars, upper, lower, number).")
        else:
...
# Gate: stop here if not authenticated
